...
/**
 * Stateless JWT security: the React app sends "Authorization: Bearer <token>" on every request.
 * Tokens are signed with HMAC-SHA256 using Spring Security's built-in Nimbus support.
 */
@Configuration
...
public class SecurityConfig {
    ...
    public SecurityConfig(SeatLockProperties props) {
        this.props = props;
    }
    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                // Lets the H2 console render in dev; harmless elsewhere.
    ...
    /** Maps the token's "roles" claim (e.g. ["ORGANIZER"]) to Spring authorities (ROLE_ORGANIZER). */
    ...
    private SecretKey jwtKey() {
        ...
        if (bytes.length < 32) {
            throw new IllegalStateException("seatlock.jwt.secret must be at least 32 bytes");
        }
